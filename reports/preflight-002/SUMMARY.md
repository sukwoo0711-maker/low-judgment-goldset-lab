...
| 단일 사실 형식을 청크당 두 번 호출 | 18청크·6사실군·24질의에서 중단 | GPU loss 이후 CPU fallback이 발생해 수율·지연 결론 금지 |

중단된 실행의 산출물은 private `.artifacts`에 보존했으며 공개 저장소에는 PID, 로컬 경로, 원시 cache를 넣지 않습니다.

v3 도중 추론 runtime이 CPU fallback으로 전환되고 GPU 진단기가 reboot-required 상태를 보고했습니다. 장애 이후 성능 표본은 폐기했으며, GPU가 정상 복구되기 전에는 후보 생성·중간 soak·4시간 soak를 재개하지 않습니다.
